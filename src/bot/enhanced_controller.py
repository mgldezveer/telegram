"""Enhanced bot controller with Redis-backed state management."""
...
class EnhancedBotController:
    ...
    async def start(self):
        ...
        try:
            ...
            await self.error_handler.handle_critical_failure(e, "EnhancedBotController")
            raise
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await self.state_manager.set_state(user_id, 'last_interaction', 'start')
            await self.state_manager.push_to_history(user_id, 'main_menu')
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /help command."""
        await self.state_manager.push_to_history(update.effective_user.id, 'help')
...
📊 Ограничение запросов: {self.rate_limiter.get_stats()}
    ...
    async def _handle_menu_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.state_manager.push_to_history(update.effective_user.id, menu)
    ...
    async def _quick_generate(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Update user state
        await self.state_manager.set_state(user_id, 'last_action', 'quick_generate')
        ...
        try:
            ...
            if result:
                ...
                # Update user state
                await self.state_manager.set_state(user_id, 'last_published_post', post.id)
            else:
                # Failed to publish
                text = (
